...
async function run() {
  ...
  const checks = {
    ...
    containsAdmissionMapping: allBodyText.includes("전형방법") && allBodyText.includes("입시결과등급/경쟁률"),
    containsParentCoaching: allBodyText.includes("학부모") && allBodyText.includes("코칭"),
  };

  const ok =
    checks.referenceDocxSections >= 10 &&
    checks.reportSectionCount >= 7 &&
